...
class AlkaliAtom(object):
    ...
    def getStateLifetimeRepopulation(self,n,l,j,temperature=0,includeLevelsUpTo=0,period=1,graph=1,thresholdState=False,detailedOutput=False):
		...
		This function creates a .txt file, plots the time evolution of the population of the Rydberg states and
		yields the lifetime values by using this fitting method: 
		https://people.duke.edu/~ccc14/sta-663/CalibratingODEs.html
		
		Written by Alessandro Greco (alessandrogreco08 at gmail dot com), 
		the simulations have been compared with experimental data ( https://arxiv.org/abs/1907.01254 )
		  
		
		Args:
					n, l, j (int,int,float): Specifies the state whose lifetime we are calculating,
						it's called the "Target" state and its color is green in the graph.
					...
					graph (int): optional. It is set to 1 by default.
						graph=0: no graph
						graph=1: it plots the population of the target (n,l,j) state with its fit and 
							it yields the value of the target lifetime in microseconds.
						graph=2: it plots the whole system (Ensemble*, Support*, Target), no fit
						graph=3: it plots the whole system (Ensemble, Support, Target) and it fits the Ensemble and Target curves, 
							it yields the values of the Ensemble lifetime and Target lifetime in microseconds.
						graph=4: it plots the whole system (Ensemble, Support, Target) + 
		...
		if ((thresholdState==False) and (graph>1)) or (thresholdState==True):
			print("Error: you need to specify the principal quantum number of the thresholdState if you use graph=2, 3 or 4.")
			return
		
		if ((graph==0) or (graph==1)):
		...
		cutoffs=int(self.getQuantumDefect(STATE,0,0.5)-self.getQuantumDefect(STATE,0,0.5))
		cutoffp05=int(self.getQuantumDefect(STATE,0,0.5)-self.getQuantumDefect(STATE,1,0.5))
		cutoffp15=int(self.getQuantumDefect(STATE,0,0.5)-self.getQuantumDefect(STATE,1,1.5))
		cutoffd15=int(self.getQuantumDefect(STATE,0,0.5)-self.getQuantumDefect(STATE,2,1.5))
		cutoffd25=int(self.getQuantumDefect(STATE,0,0.5)-self.getQuantumDefect(STATE,2,2.5))
		cutofff25=int(self.getQuantumDefect(STATE,0,0.5)-self.getQuantumDefect(STATE,3,2.5))
		cutofff35=int(self.getQuantumDefect(STATE,0,0.5)-self.getQuantumDefect(STATE,3,3.5))
		...
		NGround=self.groundStateN
		
		#It takes into account of the extra levels 
		extraL=self.extraLevels[1][:]
		...
		for pqn in xrange(extraL[0],STATE+WidthBBR+1):
			...
			#rate from s
				c[pqn+rifs,fpqn+rifp05] = self.getTransitionRate(pqn, 0, 0.5, fpqn, 1, 0.5, TEMP_BBR) #rate s -> p0.5
				c[pqn+rifs,fpqn+rifp15] = self.getTransitionRate(pqn, 0, 0.5, fpqn, 1, 1.5, TEMP_BBR) #rate s -> p1.5
			#rate from p0.5
				c[pqn+rifp05,fpqn+rifs] = self.getTransitionRate(pqn, 1, 0.5, fpqn, 0, 0.5, TEMP_BBR) #rate p0.5 -> s
				c[pqn+rifp05,fpqn+rifd15] = self.getTransitionRate(pqn, 1, 0.5, fpqn, 2, 1.5, TEMP_BBR) #rate p0.5 -> d1.5
			#rate from p1.5
				c[pqn+rifp15,fpqn+rifs] = self.getTransitionRate(pqn, 1, 1.5, fpqn, 0, 0.5, TEMP_BBR) #rate p1.5 -> s
				c[pqn+rifp15,fpqn+rifd15] = self.getTransitionRate(pqn, 1, 1.5, fpqn, 2, 1.5, TEMP_BBR) #rate p1.5 -> d1.5
				c[pqn+rifp15,fpqn+rifd25] = self.getTransitionRate(pqn, 1, 1.5, fpqn, 2, 2.5, TEMP_BBR) #rate p1.5 -> d2.5
			#rate from d1.5
				c[pqn+rifd15,fpqn+rifp05] = self.getTransitionRate(pqn, 2, 1.5, fpqn, 1, 0.5, TEMP_BBR) #rate d1.5 -> p0.5
				c[pqn+rifd15,fpqn+rifp15] = self.getTransitionRate(pqn, 2, 1.5, fpqn, 1, 1.5, TEMP_BBR) #rate d1.5 -> p1.5
				c[pqn+rifd15,fpqn+riff25] = self.getTransitionRate(pqn, 2, 1.5, fpqn, 3, 2.5, TEMP_BBR) #rate d1.5 -> f2.5
			#rate from d2.5
				c[pqn+rifd25,fpqn+rifp15] = self.getTransitionRate(pqn, 2, 2.5, fpqn, 1, 1.5, TEMP_BBR) #rate d2.5 -> p1.5
				c[pqn+rifd25,fpqn+riff25] = self.getTransitionRate(pqn, 2, 2.5, fpqn, 3, 2.5, TEMP_BBR) #rate d2.5 -> f2.5
				c[pqn+rifd25,fpqn+riff35] = self.getTransitionRate(pqn, 2, 2.5, fpqn, 3, 3.5, TEMP_BBR) #rate d2.5 -> f3.5
			#rate from f2.5
				c[pqn+riff25,fpqn+rifd15] = self.getTransitionRate(pqn, 3, 2.5, fpqn, 2, 1.5, TEMP_BBR) #rate f2.5 -> d1.5
				c[pqn+riff25,fpqn+rifd25] = self.getTransitionRate(pqn, 3, 2.5, fpqn, 2, 2.5, TEMP_BBR) #rate f2.5 -> d2.5
			#rate from f3.5
				c[pqn+riff35,fpqn+rifd25] = self.getTransitionRate(pqn, 3, 3.5, fpqn, 2, 2.5, TEMP_BBR) #rate f3.5 -> d2.5
			print (pqn, end = ' ')
		...
		while t<(totaltime):
			...
			if thresholdState!=False:
				...
				for k in range(0,riftot):
					if ((CState+rifs-cutoffs <= k < rifp05+extraL[0]) or (CState+rifp05-cutoffp05 <= k < rifp15+extraL[0]) or (CState+rifp15-cutoffp15 <= k < rifd15+extraL[0]) or (CState+rifd15-cutoffd15 <= k < rifd25+extraL[0]) or (CState+rifd25-cutoffd25 <= k < riff25+extraL[0]) or (CState+riff25-cutofff25 <= k < riff35+extraL[0]) or (CState+riff35-cutofff35 <= k < riftot)): #above the threshold state
						popall+=pop[0,k]        
		...
		if thresholdState==False:
			...
			with open("Lifetime"+str(STATE)+StrL+StrJ+".txt", 'a') as f:
				f.writelines("%.4f \t %.5f \t %.5f \t %.5f \n"%(ListTime[index],ListRed[index],ListBlue[index],ListGreen[index]) for index in range(0,len(ListTime)))
		...
		if graph==3:
			...
			report_fit(result)
			print("\n")
			print("Lifetime Ensemble: %.6f us \nLifetime Target: %.6f us"%(LifetimeEnsemble,LifetimeTarget))
		...
		if graph==4:
			def f(xs, t, ps):
				...
				x, y, z = xs
				return [+gammaTargetSpont*z + gammaSupportSpont*y, -gammaSupportSpont*y - gammaSupportBBR*y + gammaTargetBBR*z, -gammaTargetSpont*z - gammaTargetBBR*z + gammaSupportBBR*y]
